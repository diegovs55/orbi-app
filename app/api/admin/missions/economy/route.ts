/**
 * GET /api/admin/missions/economy
 *
 * Equivalente server-side de fetchMissionsForEconomy().
 ...
 * AdminNetworkEconomy aplica filtros de período client-side sobre este conjunto.
 *
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await admin
    .from("missions")
    .select(
      "id,status,service_type,total_amount,payment_method,created_at,updated_at,requester_name,selected_agent_name",
    )
    .in("status", ["cumplida", "cancelada"])
    .order("created_at", { ascending: false })
    .limit(500);
  ...
  if (error) {
    console.error("[admin/missions/economy] query error:", error);
